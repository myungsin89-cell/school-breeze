...
export default function EditTemplatePage() {
        ...
        try {
            ...
            if (data) {
                setFormData({
                    ...
                    template_type: data.template_type || 'github',
                    required_apis: data.required_apis || []
                });
            }
        } catch (error) {
            console.error('Error fetching template:', error);
            setStatusMessage({ type: 'error', text: '자료를 불러오는데 실패했습니다.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.put(`/api/templates/${params.id}`, formData);
            setStatusMessage({ type: 'success', text: '자료 수정이 완료되었습니다! 상세 페이지로 이동합니다...' });
            ...
            const errorMessage = error.response?.data?.error || error.message || '자료 수정에 실패했습니다.';
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-12">
            <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
                <Link href={`/templates/${params.id}`} className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-8 transition-colors">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="p-8 border-b border-gray-100">
                        <h1 className="text-2xl font-bold text-gray-900">자료 수정하기</h1>
                        <p className="text-gray-600 mt-2">
                            등록한 자료의 정보를 수정할 수 있습니다.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder="자료에 대한 상세한 설명을 작성해주세요."
                            />
                        </div>
                        ...
                        <div className="flex gap-3 pt-4">
                            <button
                                ...
                                className="flex-1 bg-primary text-white py-3 rounded-lg hover:bg-primary-600 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isSubmitting ? (
                                    <>
